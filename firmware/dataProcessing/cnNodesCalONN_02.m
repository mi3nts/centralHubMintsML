...
versionStrPre = 'Central_Ronn_All_ONN_2';
...
disp(graphTitle1)
display(newline)
globalCSVLabel = "resultsNowXT_CN_ONN_2"
...
% Made the edit for a different time span 
timeSpan           = seconds(mintsDefinitions.timeSpan2);
...
centralWithTargets =  retime(centralWithTargets,'regular',@nanmean,...
                    'TimeStep',timeSpan);

display("Save merged data for calibration: "+nodeID )
fileNameStr = strcat(mergedMatsFolder,"/centralWithTargets2_", nodeID,".mat");
folderCheck(fileNameStr)
save(fileNameStr,...
    'centralWithTargets')


for targetIndex = 3: length(mintsTargets)
    %     try
